...
def join_orders_customers(orders_df, customers_df):
    return orders_df.join(customers_df, customers_df.customer_id == orders_df.order_customer_id)


def count_orders_state(joined_df):
    return joined_df.groupBy('state').count().sort("count", ascending = False)
